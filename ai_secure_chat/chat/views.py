...
def chat_stream(request, chat_id):
    ...
    def sse_generator():
        ...
        # 1.5) 若启用 RAG，通知前端切换气泡为"正在调用RAG知识库..."
        ...
        try:
            ...
            for event_type, payload in provider.stream_chat(chat_entry, user_message):
                if event_type == "status":
                    yield "data: " + json.dumps({"status": payload}, ensure_ascii=False) + "\n\n"
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if chat_entry.use_rag:
                ...
                严格遵守以下全部规则：
                1.
                信息来源唯一：所有分析、数据、结论必须100 % 来自文档原文/历史数据/对话上下文，禁止编造、引用外部信息、预测股价。
                2.
                分析范围：覆盖财务表现、业务结构、成长性、盈利能力、偿债能力、经营风险、行业地位、募资用途（招股书）。
                3.
                合规底线：绝对不出现“买入、卖出、持有、加仓、减仓、推荐、翻倍、稳赚”等投资指令，只做客观分析与参考建议。
                4.
                无信息处理：文档无相关内容时，先考虑将问题拆分为可解决的子问题。若拆分后仍然无相关内容，仅回复：“根据现有文档，无法提供相关分析建议。”
                5.
                格式约束：
                若用户问及某公司的整体经营状况或投资建议等，以以下固定格式输出（必须严格执行）：
                【核心财务概况】
                【经营与成长性分析】
                【盈利能力与偿债能力】
                【核心风险提示】
                【金融参考建议（非投资建议）】
                若用户问及某公司具体财务报表、数据等细节信息，以以下格式输出（必须输出原始数据或文本，格式可调整）：
                【原始数据与文本】
                【信息分析】
                【参考投资建议】
                6.
